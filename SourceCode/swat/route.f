...
!! add transmission losses to bank storage/deep aquifer in subbasin
      ...
      !floodplain transmission loss goes to shallow aquifer storage
      if (fpinf>0) then
         sub_ha = da_ha * sub_fr(isub)
          do j = hru1(jrch), hru1(jrch) + hrutot(jrch) - 1
            shallst(j) = shallst(j) + fpinf * 86400. / (10. * sub_ha)  !floodplain infiltration added to shallow aquifer storage, mm
          end do
      endif
